...
> 2026-09-11 补充：微调实验已完成，新的可复制项目块见 `coursepilot-resume-final.md`，训练与评测讲解见 `qwen3-finetuning-explained.md`。下文保留 2026-09-07 的项目代码讲解与当时边界。
...
**CoursePilot——面向课程资料的可信个性化学习 Agent｜个人项目**

面向高校计算机课程，实现教师资料入库与审核、带引用问答、诊断测验、掌握度建模和学习路径推荐。

技术栈：Python / FastAPI / PostgreSQL / pgvector / Redis / Celery / Neo4j / BGE-M3 / BGE Reranker / Ollama / Docker Compose。

- **可信问答：**设计显式 Agent 工作流，串联输入检查、意图路由、证据门控与受限查询改写；通过结构化 Claim、引用白名单和 Grounding 校验约束生成，支持证据不足拒答与原文摘要降级。
- **检索与隔离：**实现 BGE-M3、BM25 双路召回、RRF 融合与重排，并按课程和索引覆盖集校验候选；在两门演示课程的 200 条冻结检索用例上，MRR@5 从 0.884 提升至 0.986。
- **知识与学习状态：**实现教师审核及 PostgreSQL Outbox 向 Neo4j 的异步投影；使用幂等键、唯一约束和事务保护测验作答与 Beta 掌握度更新，结合审核前置关系生成拓扑学习路径。
- **异步与本地评测：**使用 Celery/Redis 承载入库、图谱同步及评测任务，通过 Ollama 本地运行 Qwen3:4b；建立四类共 360 条内部冻结用例，并在同一组 40 条问答实验中将候选参数由 20/20/10 调为 10/8/5，观察到批次总耗时下降约 29%。

最后一条数字如果没有空间交代“实验批次”，可以删掉，把性能故事留到面试。个人贡献动词需要与你真正做过、能解释和展示的工作一致，不必声称所有代码都是逐行手写。
...
7. 引用准确率的评测归因有选择偏差风险，旧稿中的“100%”暂不作为简历主指标。第 11 节解释原因。
...
当前 `_qa_judgments` 遍历引用和标注 Claim。如果该引用不在任何对应标注的 supporting Chunk 集合中，就可能被 `continue` 跳过，不进入后续准确率分母。匹配到 supporting Chunk 的引用，又用相同成员关系设置 supports_claim。

这意味着历史 36/36 是“进入该归因判定的记录”的结果，不能稳妥地解释为“所有输出引用都正确”，更不能解释为“80 个答案全对”。它可能漏算了无法归因的错误引用。运行时词法 Grounding 也不能补上严格的语义正确性证明。

因此旧简历稿“引用准确率 100%”应暂缓使用。要恢复该指标，需要逐个生成 Claim–Citation 配对计入分母，对未知、错误、无法归因记录明确判负或单列；补反例和独立支持标注后，重新产生新版报告。不能用新口径覆盖旧报告。
