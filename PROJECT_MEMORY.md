...
2026-09-12 按用户确认的最终简历同步全部相关说明：正文统一维护在
`docs/interview/coursepilot-resume-final.md`，四条为可信问答、检索优化、数据与状态治理、模型微调。
配套解释与数字映射见 `docs/interview/coursepilot-resume-guide.md`。仅文档更新，未重跑训练、
未改动实验原始结果或默认模型；旧引用准确率撤销准入，历史结果仅供审计。
“100 条独立构造对照”是 50 个问题的有/无证据配对，不是 100 个独立问题或人工盲测。

2026-09-11 微调实验最终交付：已完成两轮真实 Qwen3-4B QLoRA 训练、180 条新合成
留出、80 条真实课程回归、50 条外部原文对照和 100 条新资料有/无证据三方案对照。
用户明确“无需证明微调优于原模型，只需证明真实理解并实践微调”，因此不继续追分训练。
第二轮 1,448 train / 236 dev，仅更新约 0.81% 参数；适配器已接入现有接口并有本机哈希一致备份。
100 条外部构造对照中，原模型与 QLoRA 正例字段覆盖同为 47/50，缺证据拒答 46/50→50/50；
但有两条事实选择退化，不声称全面提升。原模型保持默认，适配器作为实验方案保留。
最终报告：`docs/evaluation/qwen3-final-report.md`；可复制简历：`docs/interview/coursepilot-resume-final.md`。
下方早期 v1/进行中记录是历史，不是尚未完成的当前任务。
...
| 日期 | 决策 | 原因 | 影响 |
|---|---|---|---|
| 2026-09-11 | 按用户最终要求将微调定位为真实训练与技术取舍验证，不以全面效果提升为必要条件 | 用户明确无需证明微调更好；实验已体现收益、退化和评分偏差 | 完成现有对照后停止新增训练；保留默认 Ollama；以训练事实和限定评测写简历，不使用 20→39 原文匹配伪装语义提升 |
